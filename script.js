...
const I18N = {
  ...
  ru: {
    title: "Bahrom Muhamedzhanov — Product Delivery Lead · Product Leader",
    metaDescription:
      "Product Delivery Lead с продуктовым лидерским мандатом: направление портфеля, приоритизация, продуктовые ставки и лидерство в стратегии‑to‑execution в fintech, платформах, HR tech и регулируемых сценариях. Дубай.",
    nav: {
      summary: "Резюме",
      impact: "Результаты",
      cases: "Кейсы",
    ...
    hero: {
      eyebrow: "Продуктовое лидерство · Fintech · Платформы · Масштаб",
      titleLine: "Product Delivery Lead · Product Leader",
      lead1:
        "Я определяю направление и приоритизацию портфеля — а затем выстраиваю систему, которая превращает продуктовые ставки в измеримые результаты для продукта, engineering и бизнеса.",
      lead2: "Я фокусируюсь на том, какие ставки действительно важны — а не только на том, как они реализуются.",
      proof1:
        '<span class="hero-proof-num">+34%</span> скорость доставки — рост появился благодаря дисциплине портфеля и более четким компромиссам.',
      proof2:
        '<span class="hero-proof-num">−63%</span> активный бэклог — дорожная карта отражала стратегию, а не шум.',
      proof3:
        '<span class="hero-proof-num">111</span> магазинов + HQ · <span class="hero-proof-num">~1.1–1.4k</span> пользователей — отвечал за HR tech продукты, где везде должны были выдерживаться стандарты, масштаб и бизнес‑компромиссы.',
      proof4:
        '<span class="hero-proof-num">+60k</span> заявок (первый месяц) — отвечал за ипотечную инициативу роста, где вместе должны были сойтись воронка, интеграции и ops под нагрузкой.',
    ...
    exec: {
      ...
      role: "Продуктовый лидер",
      org: "Владение портфелем · Renmoney · цифровой банкинг",
      blurb:
        "Я рассматриваю продукт как портфель: определяю, какие ставки формируют следующий квартал, как приоритеты превращаются в релизы, и как продукт, engineering, аналитика и бизнес согласуют действия вокруг измеримых результатов.",
      focusLabel: "Фокус",
      focusValue:
        "Направление портфеля, приоритизация, операционный ритм, результаты в связке с бизнес‑эффектом",
      domainsLabel: "Домены",
      domainsValue:
        "Fintech, digital banking, HR tech в масштабе, партнерские платформы, привлечение",
      scaleLabel: "Масштаб",
      scaleValue:
    ...
    summary: {
      ...
      p: "Я объединяю <strong>стратегию продукта и исполнение</strong>: определяю приоритизацию портфеля и применяю продуктовые решения, затем формирую ритм, операционную модель и кросс‑функциональное взаимодействие, чтобы команды выдавали <strong>измеримые бизнес‑результаты</strong>. Делал это в <strong>digital banking</strong>, <strong>HR tech на уровне розницы</strong>, <strong>партнерских и rewards‑платформах</strong> и <strong>ипотеке и росте при больших объемах</strong> — где лидерство означает решения в масштабе портфеля, а не активность.",
    },
    impact: {
      tag: "Результаты",
      h2: "Ключевой эффект",
      ...
      items: [
        ...
        },
        {
          ...
          copy:
            "Укрепил правила intake и дисциплину приоритизации: стало меньше шума, освободилась емкость для продуктовых ставок, а компромиссы для стейкхолдеров стали явными.",
        },
        {
          metric:
            "−92% <span class=\"impact-label\">операционные затраты</span>",
          copy:
            "Заменил ручной, call-heavy маршрут на автоматизацию в точке масштаба — затраты снизились там, где численность уже не могла поглощать объем.",
        },
        {
          metric:
            "+30% <span class=\"impact-label\">привлечение кандидатов</span>",
          copy:
            "Запустил и вел referral как рычаг роста — измерял end-to-end, чтобы привлечение улучшалось без пропорционального роста затрат на acquisition.",
        },
        {
          metric: "+2 <span class=\"impact-label\">NPS кандидатов</span>",
          copy:
            "Перенес опыт и процесс в продукт, которым пользуются каждый день. NPS вырос, потому что workflow стал проще, а не из‑за разовой кампании.",
        },
        {
          metric: "111 + ~1.1–1.4k",
          copy:
            "Отвечал за продуктовые решения и приоритизацию для HR‑платформ по магазинам и HQ — надежность и единообразие стали вопросами лидерства, а не объема тикетов.",
        },
        {
          metric:
            "+300 <span class=\"impact-label\">партнеров</span>",
          copy:
            "Масштабировал onboarding партнеров и self‑serve сценарии — рост сети стал независим от линейного найма поддержки.",
        },
        {
          metric: "+60k <span class=\"impact-label\">заявок</span>",
          copy:
            "Вел ипотечный push, где объем, воронка и ops должны были держаться вместе. Цифры за первый месяц отражают согласованную работу, а не запуск одной фичи.",
        },
      ],
    },
    cases: {
      tag: "Глубина",
      h2: "Кейсы",
      lead: "Направление и эффект за один взгляд — полные доказательства в разделе Experience.",
      items: [
        {
          meta: "цифровой банкинг · Renmoney",
          title: "Трансформация поставок",
          context:
            "Поставка в digital banking была непредсказуемой: бэклог продолжал расти, а приоритеты расходились.",
          outcome:
            "<strong>+34% скорость доставки · −63% активный бэклог · выше предсказуемость поставок.</strong>",
        },
        {
          meta: "HR‑технологии · Leroy Merlin",
          title: "HR‑платформа в масштабе",
          context:
            "Рекрутинг и HR‑процессы должны работать стабильно в 111 магазинах и HQ.",
          outcome:
            "<strong>+30% привлечения кандидатов · −92% операционные затраты · +2 NPS кандидатов.</strong>",
        },
        {
          meta: "Fintech · Tinkoff Bank",
          title: "Рост партнерской платформы",
          context:
            "Рост партнеров упирался в ручной onboarding и операционные bottlenecks.",
          outcome:
            "<strong>+300 партнеров · на 80% меньше ручных усилий · операции в 2× быстрее.</strong>",
        },
        {
          meta: "Ипотека · Рост · Tinkoff + DOM RF",
      ...
      cards: [
        {
          ...
          p: "Перевожу стратегию компании в последовательный портфель продуктовых ставок: есть владельцы и метрики успеха, привязанные к выручке, рискам и времени до эффекта — без «плоского» бэклога запросов.",
        },
        {
          title: "Операционная система",
          p: "Связываю стратегию с тем, что реально выходит: как приоритизация, планирование и handoffs обеспечивают попадание в нужные ставки — ритм и дисциплина релизов в службе результатов, а не ради скорости.",
        },
        {
          title: "Решения и компромиссы",
          p: "Раннее выявляю бизнес‑компромиссы: что переносим, что сокращаем, что ставим в очередь — чтобы лидерство и команды разделяли один набор ограничений и согласованные метрики успеха.",
        },
        {
          title: "Кросс‑функциональное лидерство",
          p: "Выстраиваю единую линию продукта, engineering, аналитики, QA и бизнеса по направлению, приоритетам, доказательствам и качеству релизов — от board-level ставок до клиентских результатов.",
        },
      ],
    },
    experience: {
      tag: "Трек‑рекорд",
      h2: "Опыт",
      lead: "Где я отвечал за продуктовые решения — и какие есть подтверждения.",
      items: [
        {
          date: "фев 2025 — настоящее время",
          company: "Renmoney",
          role: "Продуктовое лидерство · Digital banking",
          summary:
            "<strong>Отвечал</strong> за направление и модель приоритизации портфеля для цифровой трансформации: операционный ритм и согласование между бизнесом, engineering, аналитикой и QA — <strong>+34% скорость доставки</strong>, <strong>−63% активный бэклог</strong>.",
          details: [
            "Вел исполнение инициатив цифровой трансформации совместно с бизнесом, engineering и интеграционными стейкхолдерами",
            "Отвечал за управление бэклогом и приоритизацию по бизнес‑эффекту, ограничениям поставки и технической сложности",
            "Улучшил поставку инженерных изменений в backend, аналитике и QA; перевел backend‑команду на Kanban",
            "Перестроил Jira workflows, структуры задач и организацию бэклога; внедрил процесс RFC",
            "Собрал модель взаимодействия между аналитиками, разработчиками и QA; стандартизировал процессы поставки между командами",
            "Унифицировал требования к API и спецификации интеграций; связал бизнес и технические команды",
          ],
          outcome:
        ...
        },
        {
          ...
          summary:
            "<strong>Вел</strong> end-to-end перезапуск корпоративного сайта — отвечал за acquisition funnel, эксперименты (A/B testing) и приоритизацию в условиях давления по выручке; <strong>+18% трафика сайта</strong> и улучшение конверсионной воронки.",
          details: [
            "Вел end-to-end перезапуск корпоративного сайта; координировал бизнес, дизайн, разработку и аналитику",
            "Переприоритизировал бэклог и рамки поставки; анализировал поведение пользователей, bottlenecks и результаты A/B тестов",
            "Фокусировался на лидогенерации и эффективности привлечения; улучшил конверсионную воронку",
        ...
        },
        {
          ...
          company: "Leroy Merlin",
          role: "Старший продуктовый менеджер · HR‑технологии",
          summary:
            "<strong>Вел</strong> HRTech продукты и внутренние платформы для <strong>111 магазинов + HQ</strong> (~<strong>1,100–1,400</strong> пользователей) — балансируя бизнес‑цели, эффективность и потребности пользователей; <strong>+30%</strong> бесплатного притока кандидатов, <strong>−92%</strong> операционных затрат, <strong>+2</strong> NPS кандидатов.",
          details: [
            "Отвечал за системы рекрутмента, workflow, требования и интеграции; стандартизировал процессы в магазинах и HQ",
            "Запустил реферальную систему «Call a Friend» — <strong>+30%</strong> бесплатный приток кандидатов",
            "Заменил call-center workflow на систему на базе чатбота — <strong>−92%</strong> операционных затрат",
            "Улучшил путь кандидата, внутренние процессы найма и NPS кандидатов (<strong>+2</strong>)",
            "Определил внутренние и внешние интеграции; построил масштабируемый data layer",
          ],
          outcome: null,
        },
        {
          ...
          summary:
            "В регулируемой банковской среде <strong>спроектировал и реализовал</strong> one-click маршрут одобрения ипотеки — где compliance, интеграции и time-to-approval определяли жизнеспособность для бизнеса.",
          details: [
            "Спроектировал и реализовал one-click ипотечное одобрение; упростил клиентский путь в регулируемой банковской среде",
            ...
            "Поддержал готовность к запуску и требования compliance",
          ],
          outcome: null,
        },
        {
          ...
          summary:
            "<strong>Вел</strong> продукт в нескольких доменах — ипотека, недвижимость, партнерская платформа, rewards и B2B growth. <strong>Ключевые результаты:</strong> <strong>+300</strong> партнеров, <strong>80%</strong> меньше усилий менеджеров, <strong>2×</strong> скорость документов, <strong>73%</strong> автоматизация через бота; ипотечная инициатива <strong>60k</strong> заявок (первый месяц), <strong>+1.79%</strong> сквозная конверсия, <strong>+0.7</strong> NPS приложения; вклад в опыт, отмеченный <strong>Global Finance</strong>.",
          details: [
            "<strong>Rewards / партнерская платформа:</strong> вел delivery; построил self-service зону для партнеров; <strong>+300</strong> новых партнеров; <strong>80%</strong> снижение усилий менеджеров",
            "Заменил бумажные workflow на электронный документооборот; <strong>2×</strong> скорость обработки документов; <strong>73%</strong> запросов автоматизировались через поддержку бота",
            "Улучшил опыт в приложении (<strong>+0.7 NPS</strong>); вывел новое рабочее место менеджера; отвечал за аналитику и интеграции",
            "<strong>Ипотека и недвижимость:</strong> вел поставку end-to-end от MVP до масштабирования; MVP продаж недвижимости за <strong>1.5 месяца</strong>",
            ...
            "Вел аналитику и интеграции с банками, разработчиками и экосистемой; обеспечивал UX для признания от Global Finance",
            "<strong>Продукт по ипотеке:</strong> интеграции с партнерскими банками и девелоперами недвижимости; улучшил UX личного кабинета клиента",
          ],
          outcome: null,
        },
      ],
    ...
    approach: {
      ...
      p: "Я держусь за <strong>решения</strong>: какие продуктовые ставки определяют дорожную карту, что мы прекращаем, и как измеряем успех. Стратегия работает, когда команды разделяют одну модель приоритизации, один ритм и метрики, которые меняются, когда <strong>мы</strong> меняем продукт — а не когда меняются слайды.",
      flow: "Ставки → компромиссы → направление → реализация → измеримые результаты",
    },
    fit: {
      tag: "Кому подойдёт",
      h2: "Кому я подхожу",
      ...
      companies: [
        "Fintech и digital banks, которые масштабируют продуктовое лидерство и дисциплину портфеля",
        "Платформы и маркетплейсы, где продуктом становятся партнеры, внутренние инструменты или регулируемые сценарии",
    ...
    credibility: {
      ...
      p1: "Работа над ипотечным продуктом, отмеченная Global Finance. Веду системы в масштабе и в distributed‑командах.",
      p2: "Сильная техническая база — Master of Software Engineering — чтобы продуктовые решения и компромиссы с engineering оставались предметными.",
      educationTitle: "Образование",
      education:
        "Бауманский государственный технический университет · Master of Software Engineering · 2009—2015",
    },
    expertise: {
      tag: "Экспертиза",
      ...
      card1: [
        "<strong>Продуктовое лидерство</strong> — стратегия, приоритизация и операционная модель: как направление превращается в портфель продуктовых ставок и ритм, который команды могут выдерживать.",
        "<strong>Домены</strong> — fintech, HR tech, платформы и рост в сложных и чувствительных контекстах.",
        "<strong>Техническая глубина</strong> там, где она нужна: APIs, интеграции и аналитика — чтобы решения с engineering оставались обоснованными и быстрыми.",
      ],
      card2Title: "Лидерство и наставничество",
      card2: [
        "Наставник в <strong>Tinkoff Fintech School</strong> — потоки, где результаты говорят сами: <strong>8 из 9</strong> участников получили офферы.",
        "<strong>Пять лет</strong> преподавал в Bauman University: вел лекции, сопровождал проекты и курировал работы, в которых были результаты, отмеченные грантами.",
      ],
    },
    contact: {
      tag: "Контакты",
      h2: "Давайте обсудим",
      p1: "Ориентируюсь на роли руководителя продуктового направления, вице‑президента по продукту и опытного продуктового лидерства — на постоянной основе — в fintech, digital banking, продуктовых платформах и других сложных высоких контекстах. Базируюсь в Дубае; готов обсуждать международные возможности в этом масштабе.",
